...
data_ip="https://script.anggakp12.my.id/ip"
checking_sc() {
...
#########################
rm -f /usr/bin/user
username=$(curl https://script.anggakp12.my.id/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://script.anggakp12.my.id/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://script.anggakp12.my.id/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# julak    
    julak="https://script.anggakp12.my.id/"
...
restart_system() {
    USRSC=$(wget -qO- https://script.anggakp12.my.id/ip | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://script.anggakp12.my.id/ip | grep $ipsaya | awk '{print $3}')
    TIMEZONE=$(printf '%(%H:%M:%S)T')
    TEXT="
<code>────────────────────</code>
<b>💉AUTOSCRIPT ANG-V6💉</b>
<code>────────────────────</code>
